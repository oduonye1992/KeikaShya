...
export default class AudioComponent extends Component {
    ...
    constructor (props){
        super(props);
        this.state = {
            pageState : 'empty', // loading, error, empty, data
    ...
    fetchRemote(_mode){
        ...
        const url = 'http://davidschristiancentre.org/wp-json/wc/v1/products?consumer_key='+AppSettings.woocommerse.consumer_key;
        fetch(url)
            .then(res => res.json())
            .then(response => {
    ...
    renderMode(){
        if (this.state.pageState == 'loading'){
            ...
            return (
                <EmptyStateComponent
                    title = "No Audio Messages are available now"
                    description = "You will be notified when a new audio message pops up"
        ...
        return (
            <Container >
                <Header style={{backgroundColor:'#232642'}}>
                    ...
                    <Title style={{color:'white', fontFamily:'AvenirNext-Medium'}}>AUDIO MESSAGES</Title>
                </Header>
                <Content
                    refreshControl={
                        <RefreshControl
