...
export default class EventsComponent extends Component {
    ...
    renderMode(){
        if (this.state.pageState == 'loading'){
            ...
            return (
                <EmptyStateComponent
                    title = "No Events are available now"
                    description = "You will be notified when a new Event pops up"
        ...
        return (
            <Container >
                <Header style={{backgroundColor:'#232642'}}>
                    ...
                    <Title style={{color:'white', fontFamily:'AvenirNext-Medium'}}>EVENTS</Title>
                    <Button
                        onPress = {() => {
                            requestAnimationFrame(() => {
                                store.dispatch({
